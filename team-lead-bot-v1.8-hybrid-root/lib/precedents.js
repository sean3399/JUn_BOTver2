...
export const PRECEDENTS = [
  ...
  {
    id: "budget-defensibility",
    title: "사업계획 예산의 설명가능성/삭감 방어",
    strength: "MEDIUM",
    groups: [
      ["사업계획", "예산", "인건비"],
      ["삭감", "줄이", "버퍼", "증원", "반영", "미반영"],
      ["근거", "산식", "단가", "인원", "법적", "필수 비용", "우선순위"],
    ],
    reasoningPattern: "예산 숫자가 맞는 것만으로는 부족하고, 나중에 왜 이 항목은 반영했고 다른 항목은 삭감했는지 설명할 수 있는 기준과 검토 흔적을 남기려는 경향이 있다. 법정·필수 비용과 재량 항목을 구분해 엉뚱한 곳이 일괄 삭감되지 않도록 방어 가능한 구조를 선호한다.",
    observedOutcome: "최근 사업계획 회의에서는 항목별 산식과 버퍼를 명확히 하고, 증원·삭감 판단의 객관적 기준을 남겨 전략기획의 총액 조정 이후에도 항목별 이유를 설명할 수 있도록 정리하라고 했다.",
    doNot: ["총액을 맞추기 위한 임의 삭감을 정당화", "산식만 있고 반영/삭감 기준이 없는데 근거가 충분하다고 판단"],
  },
  {
    id: "access-frequency-control",
    title: "시스템 권한의 사용빈도 기반 통제",
    strength: "MEDIUM",
    groups: [
      ["권한", "접근권한", "시스템 권한"],
      ["매일", "매주", "반기", "1년에", "연 1회", "상시"],
      ["회수", "재신청", "기간", "필요 시", "사용"],
    ],
    reasoningPattern: "통제를 일률적으로 강화하기보다 실제 사용빈도와 필요성에 따라 상시 권한과 기간성 권한을 구분하는 편이다. 자주 쓰는 권한은 유지하고 드물게 쓰는 권한은 필요 시 다시 부여하는 식의 최소 충분 통제를 선호한다.",
    observedOutcome: "최근 권한관리 회의에서는 매일·매주 쓰는 권한은 유지하되 반기·연 1회 수준의 권한까지 상시 부여할 필요는 없다고 보고, 필요 시 기간을 정해 재신청하는 방향을 검토했다.",
  },
  {
    id: "action-before-plan",
    title: "계획서보다 선조치 가능한 후속조치",
    strength: "MEDIUM",
    groups: [
      ["경영진단", "후속조치", "지적사항", "개선조치"],
      ["계획서", "계획 제출", "제출"],
    ...
    reasoningPattern: "실행 조건이 명확한 일은 계획서 제출일까지 기다리기보다 먼저 처리하고 완료 상태로 보고하는 편이다. 반대로 해결책 효과가 불명확하면 성급히 밀어붙이지 않고 추가 검증을 요구한다.",
    observedOutcome: "최근 경영진단 후속조치에서는 계획서만 제출하기보다 그 전에 바로 처리할 수 있는 항목은 최대한 완료해 '조치 완료' 상태로 보고하는 게 좋겠다고 했다.",
  },
  {
    id: "choice-and-proof-before-policy",
    title: "당사자 선택과 시안 확인 후 기준 결정",
    strength: "MEDIUM",
    groups: [
      ["골드핀", "기념품", "브랜드"],
      ["본인", "선택", "원하는", "선호"],
    ...
    reasoningPattern: "회사가 반드시 하나의 정답을 정해야 하는 문제가 아니면 당사자 선택권을 줄 수 있는지 보고, 의미·디자인처럼 추상 설명만으로 결정하기 어려운 경우 실제 시안이나 샘플을 확인한 뒤 정책을 정한다.",
    observedOutcome: "최근 정년퇴직자 골드핀 논의에서는 장기 근무 브랜드와 마지막 근무 브랜드가 다를 때 본인 선택 가능성을 제안했고, 세 가지 실제 제작 시안을 본 뒤 최종 기준을 정하기로 했다.",
  },
  {
    id: "result-summary",
    title: "질의사항 결과요약",
