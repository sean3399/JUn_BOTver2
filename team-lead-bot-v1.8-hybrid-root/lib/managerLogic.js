...
export function analyzeDocument(rawText, note = "") {
  ...
  const priorityByType = {
    ...
    people: { "팩트/의견": 114, "사실성": 110, "인과성": 105, "규정/기준": 95, "판단 주체": 92, "전결/책임": 90, "비례성": 85, "설명가능성": 80, "리스크": 75 },
...
- 판단 OS: ${(analysis.metaPrinciples || []).filter((x) => x.active).map((x) => x.label).join(" / ") || "기본 패턴"}
- 팩트/의견 분리: ${analysis.stanceSeparation?.summary || "확인된 사실을 우선"}
- 실제 패턴상 우선 확인 가능성이 높은 질문:\n${qs}

응답에서는 확인된 팩트와 의견을 섞지 마세요. 규정/확정 사실이 아닌 판단은 하나의 스탠스로 제시하고, 사용자가 새로운 사실을 주면 이전 의견을 방어하지 말고 즉시 업데이트하세요.
