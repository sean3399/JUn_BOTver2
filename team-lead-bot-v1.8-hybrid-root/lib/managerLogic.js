...
function junSimilarity(rule, reportType, stageKey) {
  const base = {
    "목적/문제": 86, 사실성: 90, "팩트/의견": 91, 인과성: 89,
    "가정 추적": 76, "금액 산정": 84, "규정/기준": 88, "전결/책임": 73,
    "판단 주체": 86, 설명가능성: 70, 일관성: 80, 비례성: 82, 실행성: 79,
    리스크: 62, 대안: 58, "적용 일정": 64, 선조치: 78, 전달성: 92,
  }[rule] || 60;
  let score = base;
  if (reportType === "budget" && ["가정 추적", "금액 산정", "설명가능성"].includes(rule)) score += 14;
  if (reportType === "policy" && ["규정/기준", "판단 주체", "일관성"].includes(rule)) score += 12;
  if (reportType === "people" && ["팩트/의견", "사실성", "판단 주체"].includes(rule)) score += 8;
  if (reportType === "process" && ["목적/문제", "인과성", "실행성", "비례성"].includes(rule)) score += 8;
...
function filterJunQuestions(questions, reportType, documentStage) {
  const stageKey = documentStage?.key || "working";
  const conceptAllowed = new Set(["목적/문제", "사실성", "팩트/의견", "인과성", "판단 주체", "전달성"]);
  const directionAllowed = new Set(["목적/문제", "사실성", "팩트/의견", "인과성", "규정/기준", "판단 주체", "전결/책임", "일관성", "실행성", "전달성"]);
  const allowed = stageKey === "concept" ? conceptAllowed : stageKey === "direction_approval" ? directionAllowed : null;

  const ranked = questions
    .map((q) => ({ ...q, junSimilarity: junSimilarity(q.rule, reportType, stageKey) }))
    .filter((q) => {
      if (!allowed) return true;
      if (allowed.has(q.rule)) return true;
      // 기획단계라도 문서 자체가 예산안이면 금액 산정은 실제 팀장 패턴상 예외적으로 허용합니다.
      if (reportType === "budget" && ["가정 추적", "금액 산정"].includes(q.rule)) return true;
      return false;
    })
    .filter((q) => q.junSimilarity >= 72)
    .sort((a, b) => b.junSimilarity - a.junSimilarity);

  return ranked.slice(0, stageKey === "concept" ? 2 : 3);
}
...
export function analyzeDocument(rawText, note = "") {
  ...
  const decisionOwnershipRelevant = c.decision >= 1 || c.authority >= 1 || c.userChoice >= 1 || ["policy", "people"].includes(reportType);
  ...
  const priorityByType = {
    ...
    process: { "목적/문제": 110, "인과성": 105, "실행성": 95, "비례성": 90, "선조치": 86, "일관성": 80, "판단 주체": 78, "사실성": 75, "대안": 65, "적용 일정": 60 },
    people: { "팩트/의견": 114, "사실성": 110, "인과성": 105, "전달성": 102, "규정/기준": 95, "판단 주체": 92, "전결/책임": 90, "비례성": 85, "설명가능성": 80, "리스크": 75 },
    general: { "팩트/의견": 110, "사실성": 105, "인과성": 100, "목적/문제": 95, "판단 주체": 88, "전달성": 85, "실행성": 75 },
  };
  ...
  const junQuestions = filterJunQuestions(deduped, reportType, documentStage).map((q) => ({ ...q, role: "jun" }));
  ...
  if (defensibilityRelevant) managerPatterns.push("결과보다 나중에 왜 반영·삭감했는지 설명 가능한 판단 흔적을 남기려는 경향");
  if (factOpinionRelevant) managerPatterns.push("팩트는 고정하되 의견은 서로 다른 스탠스로 보고 새 사실에 따라 수정하는 경향");
  if (decisionOwnershipRelevant) managerPatterns.push("규정·담당자·당사자·의사결정권자 중 누가 판단해야 하는지 구분하는 경향");
  if (actNowRelevant) managerPatterns.push("명확한 일은 계획보다 먼저 조치하고, 불명확한 일은 추가 확인 뒤 결정하는 경향");
  ...
  const metaPrinciples = [
    ...
    },
    {
      ...
      active: moneyDecision || quantified >= 2 || defensibilityRelevant,
      managerRule: "숫자·가정·반영/삭감 결정은 나중에도 왜 그렇게 했는지 설명 가능해야 합니다.",
      documentSignal: assumptionTraceRelevant && !assumptionTraceGood ? "핵심 가정값의 부모 근거가 아직 열려 있습니다." : defensibilityGood ? "판단을 설명할 근거 단서가 비교적 연결되어 있습니다." : "결론은 있으나 판단 흔적/상위 근거를 더 남길 여지가 있습니다.",
    ...
    },
    {
      ...
      active: c.opinionLanguage >= 1 || c.uncertainty >= 1 || c.fallback >= 1,
      managerRule: "자기 의견은 분명히 말하되 새로운 팩트나 타당한 이유가 나오면 판단을 수정합니다.",
      documentSignal: c.uncertainty >= 1 ? "현재도 확인/검토가 남아 있어 결론보다 추가 사실에 따라 의견이 바뀔 수 있는 사안입니다." : "현재 문서의 사실이 바뀌면 결론도 함께 업데이트해야 하는 유형입니다.",
      snippet: findSnippet(raw, ["의견", "제 생각", "검토", "확인 필요", "시안", "샘플", "가능성"]),
    },
    {
      key: "decisionOwner", label: "RIGHT DECISION OWNER", title: "적절한 판단 주체",
      active: decisionOwnershipRelevant,
      managerRule: "규정·담당자·당사자·상위 의사결정권자 중 누가 정해야 하는 문제인지 구분합니다.",
      documentSignal: decisionOwnershipGood ? "결정/담당 주체에 대한 단서가 확인됩니다." : "누가 판단해야 하는지 역할 구분이 더 필요합니다.",
      snippet: findSnippet(raw, ["전결", "결정권자", "담당", "본인 선택", "선택권", "승인"]),
    },
    {
      key: "actWhenClear", label: "ACT WHEN CLEAR", title: "명확하면 실행, 아니면 확인",
      active: actNowRelevant,
      managerRule: "바로 할 수 있는 일은 먼저 조치하고, 효과가 불명확하면 시안·샘플·추가 사실을 확인한 뒤 결정합니다.",
      documentSignal: c.immediateAction >= 1 ? "즉시/선조치 가능한 항목이 드러납니다." : c.uncertainty >= 1 ? "추가 검증이 남아 있어 성급한 결론보다 확인이 우선입니다." : c.planOnly >= 1 ? "계획만 잡혀 있어 제출 전에 바로 조치 가능한 항목이 있는지 볼 필요가 있습니다." : "실행 시점과 검증 수준을 상황에 맞춰 판단합니다.",
      snippet: findSnippet(raw, ["조치 완료", "바로", "즉시", "계획서", "시안", "샘플", "추가 확인"]),
    },
  ];
  ...
  return {
    ...
    junQuestions,
    advisorQuestions,
    firstTurnQuestions: junQuestions.slice(0, riskLevel === "LOW" ? 1 : 2),
    weakAreas,
    missingCritical,
    axisDetails,
    documentEvidence,
...
export function buildAnalysisPrompt(analysis) {
  ...
  const qs = junQuestions.length
    ? junQuestions.map((q, i) => `${i + 1}. [JUN 유사도 ${q.junSimilarity || "-"}] ${q.q}`).join("\n")
...
4) 한 번에 1개, 꼭 붙어 있는 경우만 최대 2개를 묻습니다. 이미 충분하면 '네 이 정도면 될 것 같습니다'처럼 끝낼 수 있습니다.
5) 팀장에게 실제로 반복 관찰된 질문보다 일반 경영컨설팅 모범답안을 우선하지 마세요.
6) 첨부 문서나 실제 선례에 없는 숫자·기한·인원 기준·정책을 만들지 마세요.
`;
}
