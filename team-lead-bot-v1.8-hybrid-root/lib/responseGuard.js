...
export function validateReply({ reply, sourceText = "", precedent = null, mode = "messenger", review = false, documentStage = null }) {
  const problems = [];
  ...
  if (review && mode === "messenger") {
    if (documentStage?.key === "concept") {
      const advisorDriftTerms = ["재무 근거표", "손익 시뮬레이션", "손익표", "ROI", "민감도 분석", "수요예측", "수요 예측", "밸류에이션", "시장규모", "시장 규모"];
      const drift = advisorDriftTerms.filter((term) => replyNorm.includes(normalize(term)));
      if (drift.length) problems.push(`기획단계 JUN이 ADVISOR 수준의 다음 단계 자료를 선제 요구함: ${drift.join(", ")}`);
    }
