...
이번 버전의 핵심은 **JUN / JUN Rewrite / ADVISOR 역할 분리**입니다.

- **JUN**: 실제 팀장님이 먼저 할 법한 반응만 짧게 출력
- **JUN Rewrite**: 팀장님이 선호할 보고 구조로 문장 수정
- **ADVISOR**: 6축·근거·Visual Review·전문 보완을 별도로 제공
- **Stage Gate**: 기획단계에서 다음 단계 수준의 재무/통제 자료를 JUN이 과잉 요구하지 않음

자세한 변경점은 `V2.2.0_ROLE_SPLIT_NOTES.md`를 참고하세요.
...
## V2.2.2 · 상황별 JUN 판단 깊이

JUN의 기본 판단 깊이는 L1이며, 단순 운영은 L0으로 낮추고 비용·규정·형평성·보안 등 영향도가 커질 때만 L2~L3으로 올립니다. `RIGHT DECISION OWNER`는 상시 질문이 아니라 규정·R&R·선택권·명시적 승인선이 실제 쟁점일 때만 작동합니다. 신규 평가·시스템·프로세스에는 `READER CLARITY`를 적용해 처음 보는 사람이 흐름과 사례를 이해할 수 있는지를 우선 확인합니다. UI의 숫자형 `JUN 유사도`는 확률처럼 오해되지 않도록 `패턴 강함/중간/약함 + L0~L3`로 변경했습니다.

자세한 내용은 `V2.2.2_ADAPTIVE_DEPTH_NOTES.md`를 참고하세요.

## V2.2.1 · 실제 수정본 파일 생성

JUN Rewrite는 수정안 카드에서 끝나지 않습니다. 같은 브라우저 세션에서 검토한 원본 `File` 객체를 잠시 보관하고, 사용자가 수정안을 채택한 뒤 **수정본 파일 생성**을 누르면 원본을 Private Blob에 다시 임시 업로드하여 채택한 문구를 원본 파일에 적용합니다. 생성이 끝나면 브라우저 다운로드가 시작되고 Blob은 즉시 정리됩니다.

- PPTX: 슬라이드 XML의 원문 텍스트를 찾아 교체하며 기존 장표 구조/스타일을 유지합니다.
- DOCX: document.xml의 원문 텍스트를 찾아 교체합니다.
- XLSX: 해당 원문이 들어 있는 셀을 찾아 수정본 텍스트로 교체합니다.
- PDF: 원본 페이지를 보존하고 근거 페이지에 `JUN Rewrite` 주석을 추가한 수정 검토본을 생성합니다. PDF 자체 문구의 직접 교체가 필요하면 편집 가능한 PPTX/DOCX 원본을 사용하는 것을 권장합니다.
- TXT/MD/CSV: 원문 문자열을 직접 교체합니다.

보안을 위해 검토용 서버 Blob은 기존과 동일하게 분석 직후 삭제합니다. 수정본 생성에 필요한 원본은 서버에 장기 저장하지 않고 현재 브라우저 세션의 File 객체만 재사용하므로, 페이지를 새로고침한 과거 검토에서 수정본을 만들려면 원본을 다시 첨부해 검토해야 합니다.
