...
`);
assert.equal(meetingStyleAnalysis.metaPrinciples?.length, 6, "V1.9 판단 OS 6개가 있어야 함");
for (const key of ["factFirst", "whyTrace", "proportionalControl", "revisableOpinion", "decisionOwner", "actWhenClear"]) {
...
assert.ok(meetingStyleAnalysis.documentEvidence.some((x) => x.key === "ownership"), "판단 주체 근거 카드가 있어야 함");
console.log("✓ V1.9 판단 OS + 설명가능성 + 판단주체 대시보드 데이터 생성");
