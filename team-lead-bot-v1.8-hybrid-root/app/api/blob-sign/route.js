...
export async function POST(request) {
  try {
    ...
    const originalName = String(body?.originalName || pathname || "");
    const size = Number(body?.size || 0);

    if (!pathname.startsWith("review/")) {
      return Response.json({ error: "유효하지 않은 업로드 경로입니다." }, { status: 400 });
    }
    if (pathname.length > 900) {
      return Response.json({ error: "파일명이 너무 깁니다. 파일명을 짧게 바꾼 뒤 다시 시도해 주세요." }, { status: 400 });
    }
