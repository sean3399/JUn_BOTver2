...
async function loadBlobFiles(blobFiles) {
  ...
  if (total > MAX_TOTAL_BLOB_BYTES) throw new Error("한 번에 검토할 수 있는 대용량 파일 합계는 최대 300MB입니다.");
  ...
  for (const descriptor of descriptors) {
    ...
    if (size > MAX_FILE_BYTES) throw new Error(`${name}: 파일당 최대 200MB까지 검토할 수 있습니다.`);
    const result = await get(pathname, { access: "private", useCache: false });
    if (!result || result.statusCode !== 200) throw new Error(`${name}: 업로드 파일을 읽지 못했습니다.`);
    const arrayBuffer = await new Response(result.stream).arrayBuffer();
    files.push({
      name,
      size: arrayBuffer.byteLength,
      type: descriptor?.type || result.blob?.contentType || "application/octet-stream",
...
export async function POST(req) {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      blobFiles = Array.isArray(body.blobFiles) ? body.blobFiles : [];
      uploadedBlobPathnames = blobFiles.map((f) => String(f?.pathname || "")).filter((p) => p.startsWith("review/"));
      mode = body.mode || "messenger";
      note = String(body.note || "").slice(0, 8000);
      historyRaw = JSON.stringify(Array.isArray(body.history) ? body.history : []);
      ...
      try {
        files = await loadBlobFiles(blobFiles);
      } catch (e) {
        return Response.json({ error: e.message }, { status: 400 });
      }
    } else {
      const formData = await req.formData();
      files = formData.getAll("file").filter((f) => f && typeof f.arrayBuffer === "function");
      mode = formData.get("mode") || "messenger";
      note = String(formData.get("note") || "").slice(0, 8000);
      historyRaw = String(formData.get("history") || "");
      ...
      if (uploadBytes > MAX_INLINE_UPLOAD_BYTES) return Response.json({ error: "대용량 파일은 Private Blob 직접 업로드 경로를 사용해 주세요." }, { status: 400 });
    }

    let docs;
    try {
      docs = await extractMany(files);
      docs = await enrichDocumentsWithVisualReview(docs);
    } catch (e) {
      return Response.json({ error: e.message }, { status: 400 });
    }
    ...
    if (uploadedBlobPathnames.length) await del(uploadedBlobPathnames).catch(() => {});
  }
}
