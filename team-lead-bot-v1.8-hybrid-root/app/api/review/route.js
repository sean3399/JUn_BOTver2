...
async function extractBlobDocuments(blobFiles) {
  ...
  for (const descriptor of descriptors) {
    ...
    if (!blobUrl) throw new Error(`${name}: 실제 Blob URL이 없습니다.`);
    ...
    const result = await readPrivateBlob(blobUrl, pathname, name, declaredSize);
    ...
    try {
      await deletePrivateBlob(blobUrl, pathname);
    } catch (error) {
...
export async function POST(req) {
  ...
  try {
    ...
    analysis.roleSeparation = {
      jun: "실제 팀장 반응 예측 · Stage Gate + 상황별 판단 깊이(L0~L3)",
      rewrite: "팀장 스타일 수정 · 원문 사실 유지",
      advisor: "6축·근거·Visual Review 기반 전문 검토",
    };
    ...
    analysis.judgmentBasis = [
      ...
      { label: "JUN 반복 패턴", weight: "핵심", detail: "최근 실제 반응에서 관찰된 간단 확인·깊이 전환·초견 이해도·비례통제 패턴" },
    ...
    let validation = validateReply({ reply, sourceText: sourceForGuard, precedent, mode, review: true, documentStage: analysis.documentStage });
    if (!validation.ok) {
      ...
      validation = validateReply({ reply, sourceText: sourceForGuard, precedent, mode, review: true, documentStage: analysis.documentStage });
    }
    ...
    if (uploadedBlobPathnames.length) {
      await Promise.allSettled(uploadedBlobPathnames.map((pathname) => deletePrivateBlob(null, pathname)));
    }
  }
}
