...
현재 문서 단계는 ${analysis?.documentStage?.label || "일반 검토"}입니다. 현재 JUN 판단 깊이는 ${analysis?.junDepth?.label || "L1 · 간단 확인"}입니다. 현재 단계보다 한 단계 앞선 자료를 새로 요구하거나 의미를 확장하지 마세요.
L0~L1이면 문장을 더 짧고 실행 가능하게 정리하고, 필요 이상의 근거·전결·리스크 문구를 새로 만들지 마세요. L2~L3이면 원문에 이미 존재하는 기준·근거·예외를 더 잘 보이게 구조화하되 새 근거를 추가하지 마세요.
