  ...
  async function uploadReviewFiles(filesForRequest) {
    ...
    try {
      for (let i = 0; i < filesForRequest.length; i++) {
        ...
        // Blob API가 실제 pathname을 응답하면 그것을 우선합니다.
        // 향후 suffix/정규화 정책이 바뀌더라도 서버가 잘못된 object를 읽지 않게 합니다.
        const actualPathname = String(uploadPayload?.pathname || signData.pathname || "");
        if (!actualPathname.startsWith("review/")) {
          throw new Error("Blob 업로드 응답에서 유효한 파일 경로를 확인하지 못했습니다.");
        }
