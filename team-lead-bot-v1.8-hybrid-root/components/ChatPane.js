...
const SIDEBAR_ITEMS = [
  { name: "팀장님 (개인 시뮬레이터)", preview: "V2.2 · JUN / Rewrite / ADVISOR", time: "지금", active: true, initials: "팀장" },
    ...
    <>
      ...
      <aside className="fixed lg:relative right-0 top-0 h-full w-[min(94vw,430px)] lg:w-[410px] shrink-0 bg-white border-l border-[#E1E1E1] z-30 shadow-xl lg:shadow-none flex flex-col">
        <div className="px-4 py-3 border-b border-[#E8E8E8] flex items-start justify-between gap-3">
          <div>
            <div className="font-semibold text-sm">ADVISOR 분석 해설</div>
            <div className="text-[11px] text-[#777] mt-0.5">6축 문서 품질 + 근거 추적 · JUN과 분리</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-4 py-4 space-y-5">
          <section>
            <div className="flex flex-wrap items-center gap-1.5 mb-2">
              ...
              {analysis.analysisScope?.label && <SmallBadge bg="#F3F0FF" fg="#654EA3">{analysis.analysisScope.label}</SmallBadge>}
            </div>
            ...
            {analysis.analysisScope?.key === "follow_up_turn" && (
              <div className="mt-2 rounded-lg border border-[#E7E7E7] bg-[#FAFAFA] p-2.5">
                <div className="text-[10px] font-semibold text-[#666]">이번 6축은 이전 점수 복사가 아니라 현재 턴 재계산</div>
                {analysis.analysisScope.currentUser && <div className="text-[9.5px] leading-relaxed text-[#777] mt-1 line-clamp-3">이번 답변 · “{analysis.analysisScope.currentUser}”</div>}
              </div>
            )}
          </section>

          {analysis.junDepth && (
            <section className="rounded-lg border border-[#DFE6F1] bg-[#FBFCFF] p-3">
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm font-medium">이번 JUN 판단 깊이</div>
                <SmallBadge bg="#EEF6FF" fg="#245E8C">{analysis.junDepth.label}</SmallBadge>
              </div>
              <div className="text-[10.5px] text-[#596477] leading-relaxed mt-1.5">{analysis.junDepth.reason}</div>
              {!!analysis.junDepth.escalators?.length && <div className="text-[9.5px] text-[#765A18] mt-2">깊이 ↑ {analysis.junDepth.escalators.join(" · ")}</div>}
              {!!analysis.junDepth.deescalators?.length && <div className="text-[9.5px] text-[#356A39] mt-1">깊이 ↓ {analysis.junDepth.deescalators.join(" · ")}</div>}
            </section>
          )}

          <section>
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium">JUN 모델링에 사용한 반복 패턴</div>
              <span className="text-[10px] text-[#8A8886]">상황에 따라 작동 강도가 달라짐</span>
            </div>
            <div className="space-y-1.5">
              {(analysis.metaPrinciples || []).map((item) => (
                <div key={item.key} className={`rounded-lg border px-2.5 py-2 ${item.active ? "border-[#D9D9EF] bg-[#F7F7FC]" : "border-[#ECECEC] bg-[#FAFAFA]"}`}>
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <span className="text-[10.5px] font-semibold">{item.title}</span>
                      <span className="text-[9px] text-[#777] ml-1.5">{item.label}</span>
                    </div>
                    <SmallBadge bg={item.active ? "#EBEBF9" : "#F0F0F0"} fg={item.active ? "#4F52A8" : "#8A8886"}>{item.intensityLabel || (item.active ? "현재 작동" : "배경 원칙")}</SmallBadge>
                  </div>
                  <div className="text-[10px] leading-relaxed text-[#555] mt-1">{item.managerRule}</div>
                  {item.active && <div className="text-[9.5px] leading-relaxed text-[#777] mt-1">↳ {item.documentSignal}</div>}
                </div>
              ))}
            </div>

            {analysis.stanceSeparation && (
              <div className="mt-2 rounded-lg border border-[#E7E7E7] bg-white p-3">
                <div className="text-[11px] font-semibold">팩트와 의견 분리</div>
                <div className="text-[10px] leading-relaxed text-[#5F5F5F] mt-1">{analysis.stanceSeparation.summary}</div>
                {analysis.stanceSeparation.factSignal && <div className="mt-2 text-[9.5px] text-[#356A39]">팩트 단서 · “{analysis.stanceSeparation.factSignal}”</div>}
                {analysis.stanceSeparation.opinionSignal && <div className="mt-1 text-[9.5px] text-[#7A5A00]">의견/추정 단서 · “{analysis.stanceSeparation.opinionSignal}”</div>}
              </div>
            )}
          </section>
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium">ADVISOR 문서 품질 6축</div>
          ...
          <section>
            ...
            <div className="space-y-2">
              <div className="rounded-lg border border-[#E7E7E7] p-3">
                ...
                <div className="space-y-2">
                  {(analysis.documentEvidence || []).slice(0, 7).map((item) => {
              ...
              {analysis.assumptionTrace?.relevant && (
                <div className="rounded-lg border border-[#E8DFCC] bg-[#FFFCF5] p-3">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <div>
                      <div className="text-[11px] font-semibold">핵심 가정값 추적</div>
                      <div className="text-[9px] text-[#85765A] mt-0.5">숫자 → 산식 → 왜 그 입력값인지까지 확인</div>
                    </div>
                    <SmallBadge bg="#FFF1C7" fg="#7A5A00">추적도 {analysis.assumptionTrace.score}</SmallBadge>
                  </div>
                  <div className="space-y-2">
                    {(analysis.assumptionTrace.items || []).slice(0, 6).map((item, i) => {
                      const st = BASIS_STYLE[item.status] || BASIS_STYLE.partial;
                      return (
                        <div key={`${item.type}-${item.value}-${i}`} className="rounded-md border border-[#EFE6D4] bg-white px-2.5 py-2">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-[10.5px] font-medium">{item.type}</span>
                            {item.value && <span className="text-[10.5px] font-semibold text-[#4C4C4C]">{item.value}</span>}
                            <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ background: st.bg, color: st.fg }}>{st.label}</span>
                          </div>
                          <div className="text-[10px] leading-relaxed text-[#6F6656] mt-1">{item.reason}</div>
                          {item.snippet && <div className="text-[9.5px] leading-relaxed text-[#8A8171] mt-1.5">“{item.snippet}”</div>}
                        </div>
                      );
                    })}
                  </div>
                  <div className="text-[9px] text-[#8A8171] mt-2">가정값이 보완되면 사실성·정당성·인과성 판단에도 함께 반영됩니다.</div>
                </div>
              )}

              <div className="rounded-lg border border-[#E7E7E7] p-3">
                <div className="text-[11px] font-semibold mb-2">JUN 학습에 사용한 반복 패턴</div>
                <div className="space-y-1.5">
          ...
          <section>
            <div className="text-sm font-medium mb-2">JUN 예상 질문</div>
            {allQuestions.length ? (
              <div className="space-y-2">
          ...
          {!!analysis.judgmentBasis?.length && (
            <section>
              <div className="text-sm font-medium mb-2">JUN 모델링 근거 우선순위</div>
              <div className="space-y-1.5">
          ...
          <div className="text-[9px] leading-relaxed text-[#8A8886] border-t border-[#EEEEEE] pt-3">
            6축은 팀장님 능력 점수가 아니라 ADVISOR의 문서 품질 진단값입니다. JUN은 상황별 판단 깊이(L0~L3)와 패턴 강도 필터를 거쳐 실제 팀장에게서 관찰된 반응만 출력합니다. Visual Review가 있는 문서는 OCR·표·차트·캡처 내용도 근거로 활용하되, 복잡한 디자인 미감은 별도 판정하지 않습니다.
          </div>
        </div>
      </aside>
    </>
...
export default function ChatPane() {
  ...
  const reviewSourceFilesRef = useRef(new Map());
  ...
  async function sendChat(text) {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: data.reply, mode, kind: "text", analysis: data.analysis }]);
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  async function cleanupUploadedBlobs(blobFiles) {
    if (!blobFiles?.length) return;
    try {
      await fetch("/api/blob-cleanup", {
        method: "POST",
        headers: authHeaders({ "content-type": "application/json" }),
        body: JSON.stringify({ pathnames: blobFiles.map((b) => b.pathname).filter(Boolean) }),
      });
    } catch (e) {}
  }

  async function uploadReviewFiles(filesForRequest) {
    const uploaded = [];
    try {
      for (let i = 0; i < filesForRequest.length; i++) {
        const file = filesForRequest[i];
        setUploadStatus(`${i + 1}/${filesForRequest.length} · ${file.name} 업로드 권한 확인 중…`);

        const signRes = await fetch("/api/blob-sign", {
          method: "POST",
          headers: authHeaders({ "content-type": "application/json" }),
          body: JSON.stringify({ originalName: file.name, size: file.size, contentType: uploadContentType(file) }),
        });
        const signData = await signRes.json().catch(() => ({}));
        if (!signRes.ok || !signData.presignedUrl) {
          const detail = [signData.error, signData.hint].filter(Boolean).join(" ");
          throw new Error(detail || "파일 업로드 권한 생성에 실패했습니다.");
        }

        setUploadStatus(`${i + 1}/${filesForRequest.length} · ${file.name} Blob에 업로드 중…`);
        ...
        let uploadBodyText = "";
        try {
          uploadBodyText = await uploadRes.text();
          if (uploadBodyText) uploadPayload = JSON.parse(uploadBodyText);
        } catch (e) {}

        if (!uploadRes.ok) {
          const detail = uploadPayload?.error?.message
            || uploadPayload?.error
            || uploadPayload?.message
            || uploadBodyText;
          throw new Error(`Blob 직접 업로드 실패 (${uploadRes.status})${detail ? `: ${String(detail).slice(0, 500)}` : ""}`);
        }

        // IMPORTANT: PUT 성공 응답이 반환한 실제 Blob URL/pathname이 유일한 source of truth입니다.
        // signData.pathname으로 조용히 fallback 하면 저장된 object와 GET 대상이 달라져 404가 날 수 있습니다.
        const actualPathname = String(uploadPayload?.pathname || "");
        const actualBlobUrl = String(uploadPayload?.url || "");
        if (!actualPathname.startsWith("review/") || !actualBlobUrl) {
          throw new Error("Blob PUT은 성공했지만 실제 url/pathname 메타데이터를 받지 못했습니다. 잘못된 경로로 검토를 계속하지 않습니다.");
        }

        uploaded.push({
          pathname: actualPathname,
          blobUrl: actualBlobUrl,
          etag: String(uploadPayload?.etag || ""),
          name: file.name,
          size: file.size,
          type: uploadContentType(file),
        });
        setUploadStatus(`${i + 1}/${filesForRequest.length} · ${file.name} 업로드 완료`);
      }
      return uploaded;
    } catch (e) {
      await cleanupUploadedBlobs(uploaded);
      throw e;
    }
  }
  ...
  async function sendReview() {
    ...
    let uploadedBlobs = [];
    try {
      // 파일 크기와 관계없이 모든 첨부는 동일한 Private Blob 경로를 사용합니다.
      // 작은 파일/큰 파일의 동작 차이 때문에 생기던 재현 불가능한 버그를 제거합니다.
      uploadedBlobs = await uploadReviewFiles(filesForRequest);
      setUploadStatus("업로드 확인 완료 · OCR/장표 Visual Review 중…");
      const res = await fetch("/api/review", {
        method: "POST",
        headers: authHeaders({ "content-type": "application/json" }),
        body: JSON.stringify({
          blobFiles: uploadedBlobs,
          mode,
          note,
      ...
      uploadedBlobs = []; // 서버가 분석 후 private blob을 삭제함
      const reviewId = `review-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      // 수정본 생성 시 원본을 다시 Blob에 올릴 수 있도록 현재 브라우저 세션에서만 File 객체를 보관합니다.
      // 서버에는 분석 종료 후 원본을 남기지 않습니다.
      reviewSourceFilesRef.current.set(reviewId, filesForRequest);
      const assistantMsg = {
        role: "assistant",
        content: data.reply,
        mode,
        ...
        review: {
          ...
          createdAt: Date.now(),
          fileNames,
          files: data.files || fileNames.map((name) => ({ name })),
          note,
          issues: data.issues || [],
      ...
      await cleanupUploadedBlobs(uploadedBlobs);
      const message = String(e?.message || e);
      if (/OIDC|credentials|Access denied|forbidden|BLOB_STORE_ID|store does not exist|store_not_found/i.test(message)) {
        setError(`파일 업로드 인증 오류: ${message} Vercel Private Blob 프로젝트/환경 연결 상태를 확인해 주세요.`);
      } else {
        setError(`파일 업로드/검토 오류: ${message}`);
      }
    } finally {
      setUploadStatus("");
      setLoading(false);
  ...
  function responseFileName(res, fallback = "JUN_수정본") {
  ...
  async function generateRevisedFile(reviewId, edits) {
    const sourceFiles = reviewSourceFilesRef.current.get(reviewId);
    if (!sourceFiles?.length) {
      throw new Error("현재 브라우저 세션에 원본 파일이 없습니다. 페이지를 새로고침한 뒤의 과거 검토라면 원본을 다시 첨부해 새 검토를 한 번 실행해 주세요.");
    }
    const accepted = (Array.isArray(edits) ? edits : []).filter((x) => x.source === "jun-rewrite" && x.text && x.originalText);
    if (!accepted.length) throw new Error("파일에 반영할 JUN Rewrite 수정안이 없습니다.");

    let uploaded = [];
    try {
      setUploadStatus("수정본 생성용 원본 업로드 중…");
      uploaded = await uploadReviewFiles(sourceFiles);
      setUploadStatus("채택한 수정안을 원본 파일에 반영 중…");
      const res = await fetch("/api/rewrite-file", {
        method: "POST",
        headers: authHeaders({ "content-type": "application/json" }),
        body: JSON.stringify({ blobFiles: uploaded, edits: accepted }),
      });
      if (!res.ok) {
        let detail = "";
        try {
        ...
        throw new Error(detail || `수정본 파일 생성 실패 (${res.status})`);
      }
      const blob = await res.blob();
      const fileName = responseFileName(res, sourceFiles.length > 1 ? "JUN_수정본_파일.zip" : `JUN_수정본_${sourceFiles[0].name}`);
      ...
      return {
        fileName,
        applied: Number(res.headers.get("x-jun-applied-edits") || 0),
        unmatched: Number(res.headers.get("x-jun-unmatched-edits") || 0),
      };
    } finally {
      setUploadStatus("");
      // rewrite-file 서버도 finally에서 정리하지만, 네트워크 중단 시 남을 수 있어 클라이언트에서도 한 번 더 정리합니다.
      if (uploaded.length) await cleanupUploadedBlobs(uploaded);
    }
  }
  ...
  function handleFilePick(e) {
    ...
    setPendingFiles((prev) => {
      ...
      const tooLarge = merged.find((f) => f.size > MAX_FILE_BYTES);
      if (tooLarge) {
        setError(`${tooLarge.name}: 파일당 최대 200MB까지 검토할 수 있습니다.`);
        return prev;
      }
      const total = merged.reduce((sum, f) => sum + f.size, 0);
      if (total > MAX_TOTAL_UPLOAD_BYTES) {
        setError("한 번에 첨부할 수 있는 파일 합계는 최대 300MB입니다. 큰 자료는 나눠서 검토해 주세요.");
        return prev;
      }
      setError(null);
      return merged;
  ...
  function resetConversation() {
    reviewSourceFilesRef.current.clear();
    if (!messages.length) {
      setMenuOpen(false);
      return;
    }
  ...
  const reviewCycles = messages.flatMap((m) => m.review ? [{ ...m.review, analysis: m.analysis }] : []);

  function patchReview(reviewId, patch) {
    setMessages((prev) => prev.map((message) => {
      if (message.review?.reviewId !== reviewId) return message;
      return { ...message, review: { ...message.review, ...patch } };
    }));
  }
    ...
    <div className="w-full h-screen flex bg-[#EEEEF2] text-[#242424] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white">
        <div className="bg-white border-b border-[#E1E1E1] px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-[#616161] truncate">JUN · Manager Simulator V2.2.2 Beta</div>
            </div>
          </div>
        ...
        <div className="bg-white border-b border-[#E1E1E1] px-5 flex gap-6 text-sm">
          {[{ id: "chat", label: "채팅" }, { id: "files", label: "Review Cycle" }].map((t) => (
        ...
        {tab === "chat" ? (
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto scrollbar-thin px-5 py-4 bg-white">
              ...
              {messages.length === 0 && (
                <div className="space-y-3 max-w-2xl">
                  <p className="text-sm text-[#616161]">보고서 파일을 첨부해서 검토받거나 방향을 물어보세요. JUN 말풍선은 실제 팀장 반응만 짧게 보여주고, <HelpCircle size={12} className="inline -mt-0.5" />는 별도 ADVISOR 분석을 엽니다.</p>
              ...
              <div className="space-y-3">
                ...
                {loading && (
                  <div className="flex justify-start gap-2"><Avatar initials="팀장" size={28} /><div className="bg-[#F5F5F5] rounded-lg rounded-tl-sm px-3.5 py-2 text-sm flex items-center gap-2 text-[#616161]"><Loader2 size={13} className="animate-spin" />{uploadStatus || "입력 중…"}</div></div>
                )}
            ...
            <div className="bg-white border-t border-[#E1E1E1] px-5 py-3">
              {!!pendingFiles.length && (
                <div className="mb-2 flex flex-col gap-1.5">
                  ...
                  <span className="text-[10px] text-[#8a8886]">파일당 200MB · 합계 300MB까지. PDF/장표는 Visual Review(OCR·표·차트·캡처)가 자동 적용됩니다. 모든 첨부 원본은 Private Vercel Blob에 임시 업로드되고 분석 종료 후 삭제되며, Visual Review에 필요한 PDF/이미지는 OpenAI API로 임시 전송 후 삭제됩니다.</span>
                </div>
              )}
          ...
          <div className="flex-1 min-h-0">
            <ReviewWorkspace reviews={reviewCycles} onPatchReview={patchReview} authHeaders={authHeaders} onGenerateRevisedFile={generateRevisedFile} />
          </div>
        )}
      </div>
