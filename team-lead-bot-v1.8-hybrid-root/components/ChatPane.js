...
const SIDEBAR_ITEMS = [
  { name: "팀장님 (개인 시뮬레이터)", preview: "V2.1 · Visual Review", time: "지금", active: true, initials: "팀장" },
...
export default function ChatPane() {
      ...
      for (let i = 0; i < filesForRequest.length; i++) {
        ...
        if (!uploadRes.ok) {
          ...
          try {
            ...
            try { detail = await uploadRes.text(); } catch (e2) {}
          }
          throw new Error(`Blob 직접 업로드 실패 (${uploadRes.status})${detail ? `: ${detail}` : ""}`);
        }
  ...
  async function sendReview() {
    ...
    let uploadedBlobs = [];
    try {
      const totalBytes = filesForRequest.reduce((sum, f) => sum + f.size, 0);
      const useDirectUpload = totalBytes > DIRECT_UPLOAD_THRESHOLD;
      let res;

      if (useDirectUpload) {
        uploadedBlobs = await uploadLargeFiles(filesForRequest);
        setUploadStatus("업로드 완료 · OCR/장표 Visual Review 중…");
        ...
        setUploadStatus("OCR/장표 Visual Review 중…");
        ...
        res = await fetch("/api/review", { method: "POST", headers: authHeaders(), body: fd });
      }
      ...
      if (/blob|oidc|signed|token|store|BLOB_STORE_ID|BLOB_WEBHOOK_PUBLIC_KEY/i.test(message)) {
        setError(`대용량 업로드 인증 오류: ${message} Vercel Private Blob 연결 후 재배포가 완료됐는지 확인해 주세요.`);
      } else {
        setError(message);
      }
    } finally {
            ...
            <div className="bg-white border-t border-[#E1E1E1] px-5 py-3">
              {!!pendingFiles.length && (
                <div className="mb-2 flex flex-col gap-1.5">
                  ...
                  <span className="text-[10px] text-[#8a8886]">파일당 200MB · 합계 300MB까지. PDF/장표는 Visual Review(OCR·표·차트·캡처)가 자동 적용됩니다. 대용량 원본은 Private Vercel Blob에 임시 업로드되고 분석 종료 후 삭제되며, Visual Review에 필요한 PDF/이미지는 OpenAI API로 임시 전송 후 삭제됩니다.</span>
                </div>
              )}
