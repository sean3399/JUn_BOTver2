  ...
  async function uploadLargeFiles(filesForRequest) {
    ...
    try {
      for (let i = 0; i < filesForRequest.length; i++) {
        ...
        setUploadStatus(`${i + 1}/${filesForRequest.length} · ${file.name} 업로드 0%`);
        const blob = await uploadPresigned(pathname, file, {
          access: "private",
          handleUploadUrl: "/api/blob-upload",
          clientPayload: JSON.stringify({ accessCode, originalName: file.name }),
          multipart: file.size > 10 * 1024 * 1024,
          onUploadProgress: (event) => {
            const pct = Math.max(0, Math.min(100, Math.round(Number(event?.percentage || 0))));
            setUploadStatus(`${i + 1}/${filesForRequest.length} · ${file.name} 업로드 ${pct}%`);
          },
        });
        uploaded.push({
          url: blob.url,
