          ...
          <section className="rounded-xl border border-[#DCDCED] bg-white p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5 flex-wrap justify-end">
                ...
                <Badge className="bg-[#EEF6FF] text-[#245E8C]">{selected?.analysis?.junDepth?.label || "L1 · 간단 확인"}</Badge>
              </div>
            </div>
            ...
            {selected?.analysis?.junDepth && (
              <div className="mt-2 rounded-lg border border-[#E6EAF2] bg-[#FBFCFF] px-3 py-2.5">
                <div className="text-[10px] font-semibold text-[#4B5570]">이번 판단 깊이 · {selected.analysis.junDepth.label}</div>
                <div className="text-[9.5px] text-[#70788A] mt-1">{selected.analysis.junDepth.reason}</div>
                {!!selected.analysis.junDepth.escalators?.length && <div className="text-[9px] text-[#765A18] mt-1.5">깊이 ↑ {selected.analysis.junDepth.escalators.join(" · ")}</div>}
                {!!selected.analysis.junDepth.deescalators?.length && <div className="text-[9px] text-[#356A39] mt-1">깊이 ↓ {selected.analysis.junDepth.deescalators.join(" · ")}</div>}
              </div>
            )}
                  ...
                  {selected.analysis.junQuestions.slice(0, 3).map((q, i) => (
                    <div key={`${q.rule}-${i}`} className="flex items-start justify-between gap-3 rounded-lg border border-[#ECECEC] px-3 py-2">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Badge className="bg-[#F3F0FF] text-[#654EA3]">패턴 {q.patternStrength || "중간"}</Badge>
                        <Badge className="bg-[#F5F5F5] text-[#666]">L{q.questionDepth ?? 1}</Badge>
                      </div>
                    </div>
                  ))}
                </div>
