                    ...
                    {["sentence", "structure", "submission"].map((intensity) => {
                      ...
                      return (
                        <div key={intensity} className="mt-2 rounded-lg border border-[#D9D9EF] bg-[#FBFBFE] p-3">
                          ...
                          <button onClick={() => acceptJunRewrite(issue, intensity)} className="mt-2 inline-flex items-center gap-1 rounded bg-[#5B5FC7] text-white px-2.5 py-1.5 text-[10.5px]"><Check size={10} />작업본에 채택</button>
                        </div>
                      );
                    })}
                  </div>
